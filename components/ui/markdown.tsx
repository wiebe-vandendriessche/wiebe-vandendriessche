...
export default function Markdown({ content, className, hideImages = false, summaryMode = false }: Props) {
  ...
  const normalized = typeof content === 'string'
    ? content.replace(/\\r\\n|\\n/g, '\n')
    : '';
  const headingAsSpan = (props: { children?: React.ReactNode }) => <span className="font-semibold">{props.children}</span>;
  const listAsSpan = (props: { children?: React.ReactNode }) => <span>{props.children}</span>;
  const liAsSpan = (props: { children?: React.ReactNode }) => <span>{' \u2022 '}{props.children}</span>;
  const pAsSpan = (props: { children?: React.ReactNode }) => <span>{props.children} </span>;
  return (
    <div className={className}>
      <ReactMarkdown
        ...
        components={{
          img: (props: React.ImgHTMLAttributes<HTMLImageElement>) => (hideImages || summaryMode) ? null : (
          ...
          ...(summaryMode ? {
            ...
            code: ({ inline, children, ...props }: { inline?: boolean } & React.ComponentProps<'code'>) => (
              inline ? (
